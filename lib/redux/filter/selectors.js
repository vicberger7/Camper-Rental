...
export const selectVisibleCampers = createSelector([selectCampers, selectLocation, selectEquipment, selectType], (campers, location, equipment, type) => {
  const filterParts = location.toLowerCase().trim().split(",").map(part => part.trim());
  const [cityFilter, countryFilter] = filterParts;
  let filteredCampers = campers;
  if (equipment) {
    filteredCampers = filteredCampers.filter(item => item.details && (item.details[equipment] || item.details[equipment] === undefined));
  }
  if (type) {
  ...
  const result = filteredCampers.filter(item => {
    ...
    return (!cityFilter || city.includes(cityFilter)) && (!countryFilter || country.includes(countryFilter));
  });
  return result;
});